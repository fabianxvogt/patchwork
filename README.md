...
Open `index.html` from a static server. `npm run dev` serves the working surface at `http://localhost:4173`. The Harbour Lights example is ready on first load; all changes stay in local storage and can be exported as a versioned `.patchwork.json` file. Timetable export is CSV.
