...
export function timetableRows(plan) { return calculate(plan).schedule.map(event => ({ time: event.start, end: event.end, stage: event.stage, act: event.name, audience: event.audience, budget: event.budget })); }
export function timetableCsv(plan) { return [['Time', 'End', 'Stage', 'Act', 'Audience', 'Budget'], ...timetableRows(plan).map(row => [row.time, row.end, row.stage, row.act, row.audience, row.budget])].map(row => row.map(value => `"${String(value).replaceAll('"', '""')}"`).join(',')).join('\n'); }
